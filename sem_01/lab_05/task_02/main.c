...
#define ACTIVE_READERS 0  // номера семафоров в наборе
...
{
    return semop(s_id, STOP_WRITE, 2) != -1;
}
...
int main()
{
    ...
    if (shmdt((void *)shcntr) == -1 || shmctl(fd, IPC_RMID, NULL) == -1 || semctl(s_id, IPC_RMID, 0) == -1)
    {
        perror("Exit error.");
        return EXIT_FAILURE;
    }
