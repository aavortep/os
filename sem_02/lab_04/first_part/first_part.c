...
char *stat_names[] = {
    ...
    "12 majflt - Количество мажорных ошибок процесса",
    "13 cmajflt - Количество мажорных ошибок дочерних процессов процесса",
...
int read_environ(FILE* dest, int pid)
{
    ...
    // записи разделены нулевыми символами ('\0')
...
int read_maps(FILE *dest, int pid)
{
    ...
    fprintf(dest, "\n+ Информация из maps (список выделенных участков памяти, используемых процессом)\n");
...
void print_page(FILE *dest, uint64_t address, uint64_t data) {
    fprintf(dest, "0x%-16lx : %-16lx %-10ld %-10ld %-10ld %-10ld\n", address,
           data & (((uint64_t)1 << 55) - 1), (data >> 55) & 1, (data >> 61) & 1,
           (data >> 62) & 1, (data >> 63) & 1);
}

void get_pagemap_info(FILE *dest, int pid) {
    fprintf(dest, "PAGEMAP\n");
    fprintf(dest, " addr : pfn soft-dirty file/shared swapped present\n");
...
void print_task(FILE *dest, int pid) {
    DIR *d;
    struct dirent *dir;
...
int main(int argc, char* argv[])
{
    ...
    FILE* my_file = fopen("res_file.txt", "w");
	...
	print_task(my_file, pid);
	read_maps(my_file, pid);
	get_pagemap_info(my_file, pid);
